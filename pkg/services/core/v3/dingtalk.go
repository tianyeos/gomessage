...
func (c *ClientActionDingtalk) RendersMessages(client *models.Client, isMerge bool, contentList []string) []any {
	...
	if isMerge {
		...
		data := format.PackDingtalkMessage(client.ExtendDingtalk.RobotKeyword, msg, false, mobiles, dingtalkIds)
		msgList = append(msgList, data)
	} else {
		for _, msg := range contentList {
			data := format.PackDingtalkMessage(client.ExtendDingtalk.RobotKeyword, msg, true, mobiles, dingtalkIds)
			msgList = append(msgList, data)
		}
	}
	return msgList
